    ...
    namespace Serialization
    {
        namespace TypeWriter
        {
            public static class SerializableClassWriter
            {
                ...
                public static void CreateSerializeClass(CSharpWriter classWriter, string className, string serializableCode, List<MemberWriteInfo> fieldWriteInfos, bool isWriteSerializableCode)
                {
                ...
                public static void WriteMaps(List<MemberWriteInfo> fieldWriteInfos, bool isWriteSerializableCode, string classType, CSharpWriter classWriter)
                {
                    List<string> codeMap = new List<string>()
                    {
                        $"_codeMap = new ({fieldWriteInfos.Count})",
                        "{"
                    };

                    string code;
                    for (int i = 0; i < fieldWriteInfos.Count; i++)
                    {
                        ...
                        codeMap.Add(WriteHandler.InsertTable($"{{nameof({fieldWriteInfos[i].Name}), {code}}},", 1));
                        _ordinals.Add(code);
                    }
                    ...
                    List<string> setMap = new List<string>()
                    {
                        ...
                        $"int code;",
                        $"_setMap = new ({fieldWriteInfos.Count});",
                        string.Empty,
                    };
                ...
                public static void WriteAllFields(List<MemberWriteInfo> memberWriteInfos, CSharpWriter classWriter)
                {
                    string type;
                    string code;
