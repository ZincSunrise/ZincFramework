    ...
    namespace Serialization
    {
        namespace Handlers
        {
            public readonly struct ArrayHandler : IEnumerableHandler
            {
                ...
                {
                    statement[index++] = WriteUtility.InsertTable(SerializeWriteTool.AppendLength(name, "Length"), RelativeIndentSize);
                    statement[index++] = WriteUtility.InsertTable(SerializeWriteTool.ArrayLoopHead(name, "Length"), RelativeIndentSize);
                    ...
                    string[] array = memberHandle.GetAppend(name + "[i]");
                ...
                public void GetConvert(string name, string[] statement, ref int index, params string[] genericType)
                {
                    ...
                    statement[index++] = WriteUtility.InsertTable(SerializeWriteTool.ArrayLoopHead("tempArray", "Length"), RelativeIndentSize);

                    statement[index++] = WriteUtility.InsertTable('{', RelativeIndentSize);

                    IMemberHandle memberHandle = IMemberHandle.GetMemberHandle(RelativeIndentSize + 1, genericType[0]);
                    string[] array = memberHandle.GetConvert("tempArray[i]");
                ...
                public void GetLength(string name, string[] statement, ref int index, params string[] genericType)
                {
                    ...
                    {
                        statement[index++] = WriteUtility.InsertTable(SerializeWriteTool.GetSingleTypeLength(name, genericType[0], "Length"), RelativeIndentSize);
                    }
                    else
                    {
                        statement[index++] = WriteUtility.InsertTable(SerializeWriteTool.ArrayLoopHead(name, "Length"), RelativeIndentSize);
