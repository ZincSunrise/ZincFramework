...
namespace ZincFramework
{
    namespace Json
    {
        ...
        public class JsonDataManager : BaseSafeSingleton<JsonDataManager>
        {
            ...
            public void SaveData(object data, string saveName, Type type, bool isPrettyPrint = false, JsonSerializeType jsonType = JsonSerializeType.JsonSerializer)
            {
                AppendPath(saveName);
                using (FileStream fileStream = File.Create(_pathBulider.ToString(), 1024))
                {
                    JsonSelector.Serialize(data, type, isPrettyPrint, fileStream, jsonType);
                }

                RestorePath();
            }

            public void SaveData<T>(T data, string saveName, bool isPrettyPrint = false, JsonSerializeType jsonType = JsonSerializeType.JsonSerializer)
            {
                AppendPath(saveName);
                using (FileStream fileStream = File.Create(_pathBulider.ToString(), 1024))
                {
                    JsonSelector.Serialize<T>(data, fileStream, isPrettyPrint, jsonType);
                }
