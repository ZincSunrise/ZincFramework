        ...
        namespace Binary
        {
            public readonly struct DictionaryWriter : ISerializeWrite
            {
                public void Write(object obj, Stream stream, Type type)
                {
                    ...
                    {
                        keyWriter.Write(key, stream, keyType);
                        valueWriter.Write(dictionary[key], stream, valueType);
                    }
                }
                ...
                public async Task WriteAsync(object obj, Stream stream, Type type)
                {
                    ...
                    foreach (object key in dictionary.Keys)
                    {
                        await keyWriter.WriteAsync(key, stream, keyType);
                        await valueWriter.WriteAsync(dictionary[key], stream, valueType);
                    }
                }
            }   
        }
